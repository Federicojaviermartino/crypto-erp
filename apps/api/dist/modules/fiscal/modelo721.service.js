...
Object.defineProperty(exports, "Modelo721Service", {
    enumerable: true,
    get: function() {
        return Modelo721Service;
    }
});
...
let Modelo721Service = class Modelo721Service {
    /**
   ...
   */ async generateModelo721(companyId, year) {
        ...
        const balancesBySource = new Map();
        ...
        for (const wallet of wallets){
            ...
            for (const tx of wallet.transactions){
                ...
                if (tx.assetIn && tx.amountIn) {
                    ...
                    if (!balancesBySource.has(sourceKey)) {
                        balancesBySource.set(sourceKey, {
                            asset: tx.assetIn,
                            assetName: tx.assetIn,
                            source: `WALLET-${wallet.address.slice(0, 8)}`,
                            sourceName: wallet.label || `Wallet ${wallet.chain}`,
                            country,
                            balance: new _decimal.default(0),
                            costBasis: new _decimal.default(0),
                            firstAcquisition: null
                        });
                    }
                    const entry = balancesBySource.get(sourceKey);
                    const amount = new _decimal.default(tx.amountIn);
                    const priceEur = tx.priceInEur ? new _decimal.default(tx.priceInEur) : new _decimal.default(0);
                ...
                if (tx.assetOut && tx.amountOut) {
                    ...
                    const entry = balancesBySource.get(sourceKey);
                    const amount = new _decimal.default(tx.amountOut);
        ...
        for (const exchange of exchangeAccounts){
            ...
            for (const lot of exchangeLots){
                const remaining = new _decimal.default(lot.remainingAmount);
                ...
                entry.costBasis = entry.costBasis.plus(new _decimal.default(lot.costBasisEur));
        ...
        for (const [, data] of balancesBySource){
            ...
            const priceEur = pricesAtYearEnd.get(data.asset) || new _decimal.default(0);
        ...
        const totalValorEur = posiciones.reduce((sum, p)=>sum.plus(p.valorEurAFinDeAno), new _decimal.default(0));
        ...
        this.logger.log(`Modelo 721 generado para ${year}: ${posiciones.length} posiciones, ` + `valor total: ${totalValorEur.toFixed(2)} EUR, obligado: ${summary.obligadoDeclarar}`);
        return summary;
    }
    /**
   ...
   */ async generateModelo720Crypto(companyId, year) {
   ...
   */ async exportToAEATFormat(companyId, year) {
   ...
   */ async exportModelo720ToAEATFormat(companyId, year) {
        ...
        const totalValor = items.reduce((sum, item)=>sum.plus(item.valoracionEur), new _decimal.default(0));
        const xmlParts = [];
        xmlParts.push('<?xml version="1.0" encoding="UTF-8"?>');
   ...
   */ async getPricesAtDate(date) {
        ...
        for (const record of priceRecords){
            if (!seenSymbols.has(record.symbol)) {
                ...
                prices.set(record.symbol, new _decimal.default(record.priceEur));
            }
        }
        ...
        for (const asset of assets){
            if (!prices.has(asset.symbol)) {
                ...
                prices.set(asset.symbol, new _decimal.default(fallbackPrice));
                this.logger.warn(`No hay precio histórico para ${asset.symbol} a ${this.formatDate(date)}, usando fallback: ${fallbackPrice} EUR`);
            }
        }
        return prices;
    }
   ...
   */ async getPreviousYearTotal(companyId, year) {
   ...
   */ async validateForSubmission(companyId, year) {
        const modelo = await this.generateModelo721(companyId, year);
        const errors = [];
        const warnings = [];
        ...
        for (const pos of modelo.posiciones){
            ...
            if (ratio.gt(100) || ratio.lt(0.01)) {
                warnings.push(`${pos.tipoMoneda}: Diferencia extrema entre valor actual (${pos.valorEurAFinDeAno.toFixed(2)} EUR) ` + `y coste (${pos.valorAdquisicionEur.toFixed(2)} EUR) - verificar precios`);
            }
        }
        // Validaciones globales
        if (!modelo.obligadoDeclarar) {
            warnings.push(`Valor total (${modelo.totalValorEur.toFixed(2)} EUR) no supera umbral de 50,000 EUR - ` + `no hay obligación de declarar el Modelo 721`);
        }
        ...
        if (priceCheck === 0) {
            warnings.push(`No hay precios históricos para la fecha 31/12/${year}. ` + `Los valores mostrados son estimaciones y deben verificarse.`);
        }
        return {
   ...
   */ async getSummary(companyId, year) {
        ...
        for (const pos of modelo.posiciones){
            const current = byAsset.get(pos.tipoMoneda) || new _decimal.default(0);
            byAsset.set(pos.tipoMoneda, current.plus(pos.valorEurAFinDeAno));
        }
        // Agrupar por exchange
        const byExchange = new Map();
        for (const pos of modelo.posiciones){
            const current = byExchange.get(pos.nombreExchange) || {
                valor: new _decimal.default(0),
                pais: pos.paisExchange
            };
            current.valor = current.valor.plus(pos.valorEurAFinDeAno);
            byExchange.set(pos.nombreExchange, current);
        }
        // Top 5 assets
        const topAssets = Array.from(byAsset.entries()).sort((a, b)=>b[1].minus(a[1]).toNumber()).slice(0, 5).map(([symbol, valor])=>({
                symbol,
                valorEur: valor.toFixed(2),
                porcentaje: modelo.totalValorEur.gt(0) ? valor.div(modelo.totalValorEur).mul(100).toFixed(1) : '0'
            }));
        // Top 5 exchanges
        const topExchanges = Array.from(byExchange.entries()).sort((a, b)=>b[1].valor.minus(a[1].valor).toNumber()).slice(0, 5).map(([nombre, data])=>({
                nombre,
    ...
    constructor(prisma){
        this.prisma = prisma;
        this.logger = new _common.Logger(Modelo721Service.name);
    }
};
Modelo721Service = _ts_decorate([
...
//# sourceMappingURL=modelo721.service.js.map