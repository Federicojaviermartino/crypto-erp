...
Object.defineProperty(exports, "LibroRegistroService", {
    enumerable: true,
    get: function() {
        return LibroRegistroService;
    }
});
...
let LibroRegistroService = class LibroRegistroService {
    /**
   * Generate Libro Registro data for a company and period
   */ async generateLibroRegistro(companyId, startDate, endDate) {
        this.logger.log(`Generating Libro Registro for company ${companyId} from ${startDate.toISOString()} to ${endDate.toISOString()}`);
        // Get company
        ...
        if (!company) {
            throw new _common.NotFoundException(`Company ${companyId} not found`);
        }
        // Get issued invoices (facturas emitidas)
        const issuedInvoices = await this.prisma.invoice.findMany({
            where: {
                companyId,
                direction: _client.InvoiceDirection.SALE,
                issueDate: {
                    gte: startDate,
                    lte: endDate
                },
        ...
        const receivedInvoices = await this.prisma.invoice.findMany({
            where: {
                companyId,
                direction: _client.InvoiceDirection.PURCHASE,
                issueDate: {
                    gte: startDate,
                    lte: endDate
                },
        ...
        const facturasEmitidas = issuedInvoices.map((invoice)=>this.invoiceToLibroEntry(invoice));
        const facturasRecibidas = receivedInvoices.map((invoice)=>this.invoiceToLibroEntry(invoice));
        // Calculate totals
   ...
   */ invoiceToLibroEntry(invoice) {
        // Calculate totals
        let baseImponible = new _library.Decimal(0);
        let cuotaIVA = new _library.Decimal(0);
        invoice.items.forEach((item)=>{
            const itemBase = new _library.Decimal(item.quantity).times(item.unitPrice);
        ...
        return {
            ...
            cuotaIVA,
            total,
            retencionIRPF: invoice.withholdingAmount ? new _library.Decimal(invoice.withholdingAmount) : undefined,
    ...
    /**
   * Calculate totals
   */ calculateTotales(facturasEmitidas, facturasRecibidas) {
        const sumEntries = (entries)=>{
            return entries.reduce((acc, entry)=>({
                ...
                baseImponible: new _library.Decimal(0),
                cuotaIVA: new _library.Decimal(0),
                total: new _library.Decimal(0)
            });
        };
        return {
    ...
    /**
   * Export to CSV
   */ async exportToCSV(companyId, startDate, endDate, type = 'emitidas') {
    ...
    /**
   * Export to Excel
   */ async exportToExcel(companyId, startDate, endDate, filePath) {
        const data = await this.generateLibroRegistro(companyId, startDate, endDate);
   ...
   */ addSheetData(sheet, entries, title, data) {
        // Title row
   ...
   */ addResumenSheet(sheet, data) {
        ...
        sheet.addRow([]);
        // Balance
        const balanceBase = data.totales.emitidas.baseImponible.minus(data.totales.recibidas.baseImponible);
    ...
    constructor(prisma){
        this.prisma = prisma;
        this.logger = new _common.Logger(LibroRegistroService.name);
    }
};
LibroRegistroService = _ts_decorate([
...
//# sourceMappingURL=libro-registro.service.js.map