...
Object.defineProperty(exports, "TransactionParser", {
    enumerable: true,
    get: function() {
...
let TransactionParser = class TransactionParser {
    parseTransaction(tx, walletAddress, chain) {
        ...
        const result = this.detectFromDexSwap(tx, walletLower, chain) || this.detectFromStaking(tx, walletLower, chain) || this.detectFromBridge(tx, walletLower, chain) || this.detectFromLogs(tx, walletLower, chain) || this.detectFromValue(tx, walletLower, chain);
        // Add gas fee to result
        result.feeAsset = this.getNativeSymbol(chain);
        result.feeAmount = this.calculateGasFee(tx);
        return result;
    }
    detectFromDexSwap(tx, walletAddress, chain) {
    ...
    detectFromStaking(tx, walletAddress, chain) {
        const toAddress = tx.to_address?.toLowerCase();
        if (toAddress && STAKING_PROTOCOLS[toAddress]) {
            const protocol = STAKING_PROTOCOLS[toAddress];
            // Check for staking/unstaking events
            const hasStakeEvent = tx.log_events.some((e)=>e.decoded?.name?.toLowerCase().includes('stake') || e.decoded?.name?.toLowerCase().includes('submit') || e.decoded?.name?.toLowerCase().includes('deposit'));
            const hasUnstakeEvent = tx.log_events.some((e)=>e.decoded?.name?.toLowerCase().includes('unstake') || e.decoded?.name?.toLowerCase().includes('withdraw') || e.decoded?.name?.toLowerCase().includes('redeem'));
            if (hasUnstakeEvent) {
                return {
                    type: 'UNSTAKE',
                    subtype: protocol,
    ...
    detectFromBridge(tx, walletAddress, chain) {
        const toAddress = tx.to_address?.toLowerCase();
    ...
    detectFromLogs(tx, walletAddress, chain) {
        // Check for liquidity operations
        const hasLiquidityAdd = tx.log_events.some((e)=>e.decoded?.name === 'Mint' || e.decoded?.name === 'AddLiquidity' || e.decoded?.name?.toLowerCase().includes('liquidity') && e.decoded?.name?.toLowerCase().includes('add'));
        const hasLiquidityRemove = tx.log_events.some((e)=>e.decoded?.name === 'Burn' || e.decoded?.name === 'RemoveLiquidity' || e.decoded?.name?.toLowerCase().includes('liquidity') && e.decoded?.name?.toLowerCase().includes('remove'));
        if (hasLiquidityAdd) {
            return {
                type: 'LIQUIDITY_ADD',
                confidence: 0.85,
        ...
        const hasClaimEvent = tx.log_events.some((e)=>e.decoded?.name?.toLowerCase().includes('claim') || e.decoded?.name?.toLowerCase().includes('reward') || e.decoded?.name === 'Harvested' || e.decoded?.name === 'RewardPaid');
        if (hasClaimEvent) {
            const transfers = this.extractTokenTransfers(tx.log_events, walletAddress);
    ...
    detectFromValue(tx, walletAddress, chain) {
        const nativeSymbol = this.getNativeSymbol(chain);
    ...
    extractSwapDetails(logs, walletAddress, chain) {
        // Look for swap events
        const swapEvent = logs.find((e)=>e.decoded?.name === 'Swap' || e.decoded?.name === 'TokenExchange' || e.decoded?.name === 'Fill');
    ...
    extractTokenTransfers(logs, walletAddress) {
        const transfers = [];
    ...
    parseTransactions(transactions, walletAddress, chain) {
        return transactions.map((tx)=>this.parseTransaction(tx, walletAddress, chain));
    }
    constructor(){
        this.logger = new _common.Logger(TransactionParser.name);
    }
};
TransactionParser = _ts_decorate([
