...
Object.defineProperty(exports, "CostBasisService", {
    enumerable: true,
    get: function() {
...
let CostBasisService = class CostBasisService {
    /**
   * Create a new cost basis lot for an acquisition (BUY, TRANSFER_IN, STAKING_REWARD, etc.)
   */ async createLot(companyId, sourceTxId, cryptoAssetId, amount, costBasisEur, acquiredAt) {
   ...
   */ async calculateFIFO(companyId, cryptoAssetId, amountToSell, saleDate, saleProceeds) {
        ...
        for (const lot of lots){
            ...
            const holdingPeriod = Math.floor((saleDate.getTime() - lot.acquiredAt.getTime()) / (1000 * 60 * 60 * 24));
            lotsUsed.push({
                lotId: lot.id,
        ...
        if (remainingToSell > 0.00000001) {
            this.logger.warn(`Insufficient lots to cover sale of ${amountToSell}. Remaining: ${remainingToSell}`);
        }
   ...
   */ async getPortfolioPositions(companyId) {
        ...
        for (const asset of assets){
            ...
            const totalCostBasis = lots.reduce((sum, lot)=>sum + lot.remainingAmount.toNumber() * lot.costPerUnit.toNumber(), 0);
            positions.push({
                assetId: asset.id,
                symbol: asset.symbol,
                name: asset.name,
   ...
   */ async getLotsForAsset(companyId, cryptoAssetId, includeExhausted = false) {
   ...
   */ async generateTaxReport(companyId, startDate, endDate) {
   ...
   */ async recalculateForAsset(companyId, cryptoAssetId) {
    ...
    constructor(prisma){
        this.prisma = prisma;
        this.logger = new _common.Logger(CostBasisService.name);
    }
};
CostBasisService = _ts_decorate([
...
//# sourceMappingURL=cost-basis.service.js.map